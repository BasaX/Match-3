            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                self.highlightedTile = nil
                   ...
                   -- local check1 = self.board:checkForMatches() --self.board:checkForMatches(self.highlightedTile, x, y, self.level)
                    ...
                    --posicion actual cambia por a donde voy
                    self.highlightedTile.gridX = newTile.gridX
                    self.highlightedTile.gridY = newTile.gridY
                    --a donde voy cambia por posicion actual
                    newTile.gridX = tempX
                    newTile.gridY = tempY

                    -- swap tiles in the tiles table
                    -- en tablero cargo en posicion a donde voy valor actual del Tile
                    self.board.tiles[self.highlightedTile.gridY][self.highlightedTile.gridX] =
                        self.highlightedTile

                    -- en tablero coloco el Tile a donde voy en posicion actual
...
function PlayState:calculateMatches()
    ...
    -- if no matches, we can continue playing
        ...
        if self.highlightedTile then
          -- posicion actual
          local tempX2 = self.highlightedTile.gridX
          local tempY2 = self.highlightedTile.gridY
          
          --adonde voy
          local newTile2 = self.board.tiles[newTile.gridY][newTile.gridX]
          
          
          --posicion actual cambia por a donde voy
          self.highlightedTile.gridX = newTile2.gridX
          self.highlightedTile.gridY = newTile2.gridY
          --a donde voy cambia por posicion actual
          newTile2.gridX = tempX2
          newTile2.gridY = tempY2
          ...
          self.boardHighlightX = newTile2.x / 32
          self.boardHighlightY = newTile2.y / 32
          -- tween coordinates between the two so they swap
          Timer.tween(0.1, {
             [self.highlightedTile] = {x = newTile2.x, y = newTile2.y},
             [newTile2] = {x = self.highlightedTile.x, y = self.highlightedTile.y}
             })
                :finish(function() self.canInput = true self.highlightedTile = nil end)
      else  
        self.canInput = true
      end
    end
...
function PlayState:render()
    ...
    if self.board.drawHorizontalEffect then    
      love.graphics.setColor(1, 1, 1, self.board.transitionAlpha)
      love.graphics.rectangle('fill', self.board.xEffect, self.board.yEffect, VIRTUAL_WIDTH, 24)
    end

    if self.board.drawVerticalEffect then    
      love.graphics.setColor(1, 1, 1, self.board.transitionAlpha)
      love.graphics.rectangle('fill', self.board.xEffect, self.board.yEffect, 24, VIRTUAL_HEIGHT)
    end
end