...
function StartState:update(dt)
    ...
    if not self.pauseInput then
        ...
        if mouseX ~= nil and mouseY ~= nil then
            if (mouseX == VIRTUAL_WIDTH and mouseY == (VIRTUAL_HEIGHT / 2 + 12 + 8)) then
                x = VIRTUAL_WIDTH
                y = VIRTUAL_HEIGHT / 2 + 12 + 8
                self.currentMenuItem = 1
            else
                mouseX = nil
                mouseY = nil
            ...
            if (mouseX == VIRTUAL_WIDTH and mouseY == (VIRTUAL_HEIGHT / 2 + 33 + 8)) then
              x = VIRTUAL_WIDTH
              y = VIRTUAL_HEIGHT / 2 + 33 + 8
